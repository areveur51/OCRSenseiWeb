...
### OCR Processing
OCR processing is managed by a Python-based service using dual OCR verification with `pytesseract` (Tesseract 5.3.4 LSTM) and EasyOCR. Images are preprocessed using OpenCV (grayscale, upscaling, denoising, binarization, auto-deskewing) before OCR analysis. The system runs two different configurable Tesseract PSM modes (default: PSM 6 and PSM 3) concurrently via ThreadPoolExecutor. **EasyOCR consistently outperforms Pytesseract** (avg confidence 65.75 vs 53.65) and is prioritized when confidences are equal. Results are labeled as "pytesseract" or "easyocr" in consensus_source. Word-level bounding boxes are extracted for frontend highlighting.
